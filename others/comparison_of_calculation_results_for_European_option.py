...
theoretical_price_OU_model_case = np.exp(mu_hat + sigma_hat_p2 / 2) * norm.sf(
    (np.log(K) - mu_hat - sigma_hat_p2) / np.sqrt(sigma_hat_p2), loc=0, scale=1) - K * norm.sf(
    (np.log(K) - mu_hat) / np.sqrt(sigma_hat_p2), loc=0, scale=1)
...
def sum_probability_for_weight(k=3, position=3):
    if k in sum_probability_dic.keys():
        if position in sum_probability_dic[k].keys():
            ...
            if position == k:
                sum_probability = p(n, T, alpha, sigma, x_0 + weight_deltas(k - 1)[k - 1]) * sum_probability_for_weight(
                    k - 1, k - 1)
            elif position == 0:
                sum_probability = (1 - p(n, T, alpha, sigma,
                                         x_0 + weight_deltas(k - 1)[0])) * sum_probability_for_weight(k - 1, 0)
                ...
                if k - 1 in sum_probability_dic.keys():
                    ...
                    sum_probability2 = sum_probability_for_weight(k - 1, position - 1)
                    sum_probability_dic[k - 1][position - 1] = sum_probability2
                sum_probability = p(n, T, alpha, sigma, x_0 + weight_deltas(k - 1)[position - 1]) * \
                                  sum_probability_dic[k - 1][position - 1] + (
                                          1 - p(n, T, alpha, sigma, x_0 + weight_deltas(k - 1)[position])) * \
                                  sum_probability_dic[k - 1][position]
            sum_probability_dic[k][position] = sum_probability

    else:
        if position == k:
            sum_probability = p(n, T, alpha, sigma,
                                x_0 + weight_deltas(k - 1)[position - 1]) * sum_probability_for_weight(k - 1,
                                                                                                       position - 1)
        elif position == 0:
            sum_probability = (1 - p(n, T, alpha, sigma, x_0 + weight_deltas(k - 1)[0])) * sum_probability_for_weight(
                k - 1, 0)
        else:
            sum_probability1 = sum_probability_for_weight(k - 1, position)
            sum_probability_dic[k - 1] = {position: sum_probability1}
...
def theoretical_price_discrete_OU_model_case(n=100, time=0.01, K=0.2):
    k = int(n * time)
...
for n in range(100, 301, 10):
    ...
    # discrete OU model case theoretical value
    delta = discrete_OU_model_generator.Delta(n, T, sigma)
    sum_probability_dic = {1: {0: 1 - p(n, T, alpha, sigma, x_0), 1: p(n, T, alpha, sigma, x_0)}}
    sum_probability = None

    discrete_part = theoretical_price_discrete_OU_model_case(n, T, K)
